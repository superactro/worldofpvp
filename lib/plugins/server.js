ig.module(
    ...
    .defines( function () {
        Server = ig.Game.extend( {
            ...
            init: function () {
                ...
                ig.io.sockets.on( 'connection', function ( socket ) {
                    // "socket.on" sets "this" to "socket"
                    socket.on( 'disconnect', function () {
                        serverThis.clientDisconnected( this );
                    }).on( 'reconnect', function () {
                            serverThis.clientReconnected( this );
                    ...
                    serverThis.clientConnected( socket );
                });
                // Every second calculate the average ping for clients.
                this.clientInterval = setInterval( function () {
                    var total = 0;
                    var i = 0;
                    for ( var key in serverThis.clients ) {
                        if ( !serverThis.clients[key] ) continue;
                        total += serverThis.clients[key].latency.avg;
                        i++;
                    }
                    serverThis.clientAvgPing = parseInt( total / i );
                }, 1000 );
            },
            ...
            clientConnected: function ( socket ) {
                ...
                ig.game.entities.forEach( function ( ent ) {
                    serverThis.entityCreate( ent.classType, ent.pos.x, ent.pos.y, ent._settings, socket );
                    serverThis.entityMove( ent, socket );
                });
        ...
        ig.main = function ( canvasId, gameClass, fps, width, height, scale, loaderClass ) {
            ...
            ig.ready = true;
            loaderClass = loaderClass || ig.Loader;
